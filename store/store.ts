      ...
      onLogin: async () => {
        try {
          ...
          if (profileRes.ok) {
            ...
            // Don't show error toast here since useAuthSync already sets auth state
          }
          ...
          if (res.ok) {
            ...
            dbCart = data.cart || [];
            console.log("🛒 Database cart loaded:", dbCart);
          }

          // 3. Get the current guest cart
          const guestCart = get().cart;
          console.log("🛍️ Guest cart:", guestCart);
          ...
          // 7. Update the local state with merged cart AND user profile
          // Only update auth.user if we successfully got the profile
          if (userProfile) {
            set((state) => ({
              cart: mergedCart,
              total: newTotal,
              cartSynced: !hasNewItems, // Only mark as synced if no new items to sync
              auth: {
                ...state.auth,
                user: userProfile, // Update user profile here
              },
            }));
          } else {
            // Just update cart if no profile
            set({
              cart: mergedCart,
              total: newTotal,
              cartSynced: !hasNewItems,
            });
          }
          ...
          // Don't modify auth state here - let useAuthSync handle it
          // Just load fallback cart
          try {
            await get().loadCartFromServer();
          } catch (fallbackError) {
