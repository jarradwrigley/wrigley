...
//     if (status === "EXPIRED" || status === "DECLINED") return "#ef4444"; // red
  ...
  const handleActivateClick = async () => {
    ...
    try {
      ...
      toast.error("Activation Failed", {
        description: err.message || "Failed to initiate activation process",
      });
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      if (response.ok) {
        toast.success("Subscription Activated!", {
          description: `${subscription.deviceName} is now active and encrypted`,
        });
        onActivationSuccess();
        setShowActivation(false);
        setTotpCode("");
