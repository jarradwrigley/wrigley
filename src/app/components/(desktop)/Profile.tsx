...
const Content: React.FC = () => {
  ...
  useEffect(() => {
    fetchProfile();
    fetchSubscriptions();
  }, [fetchProfile, fetchSubscriptions]);
  ...
  const handleSaveProfile = useCallback(
    async (updatedProfile: Profile) => {
      try {
        ...
        const updatedData = await response.json();
        setProfile(updatedData.profile || updatedProfile);

        await updateUserProfile();
        await fetchSubscriptions(); // Refresh subscriptions if needed
    ...
    [updateUserProfile, fetchSubscriptions]
  );
      ...
      <div className="flex w-full items-center justify-center h-64">
        <div className="text-center">
          <p className="text-red-500 mb-2">
            {error || subscriptionError || "Failed to load profile"}
          </p>
          <button
            onClick={() => {
              fetchProfile();
