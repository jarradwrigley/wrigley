...
export default function GuestOrderTracking() {
  ...
  useEffect(() => {
    const checkForConfirmationData = () => {
      try {
        ...
        if (confirmationData) {
          ...
          if (Date.now() - timestamp < oneHour) {
            ...
            // Auto-load the order
            trackOrderInternal(storedOrderNumber, storedEmail);

            // Clear the session storage after use
            sessionStorage.removeItem("guestOrderConfirmation");
          } else {
  ...
  const trackOrderInternal = async (orderNum: string, emailAddr: string) => {
    ...
    try {
      ...
      setOrder(null);
      setIsInitialLoad(false);
      setIsConfirmationMode(false); // Switch back to tracking mode on error
    } finally {
      setLoading(false);
    }
  };
  ...
  const trackOrder = async (e: any) => {
    e.preventDefault();
    setIsConfirmationMode(false); // Manual tracking
      ...
      <div className="max-w-4xl mx-auto px-4 pt-4">
        ...
        {order && (
          <div className="bg-white rounded-lg shadow-md overflow-hidden">
            ...
            {/* Rest of the order details sections remain the same */}
